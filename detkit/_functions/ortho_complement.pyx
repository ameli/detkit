...
cpdef ortho_complement(X, Xp, X_orth):
    """
    ...
    if data_type_name == b'float32':
        _pyc_ortho_complement_float(X, Xp, X.shape[0], X.shape[1], Xp.shape[1],
                                    int(X_orth))
    elif data_type_name == b'float64':
        _pyc_ortho_complement_double(X, Xp, X.shape[0], X.shape[1],
                                     Xp.shape[1], int(X_orth))
    elif data_type_name == b'float128':
        _pyc_ortho_complement_long_double(X, Xp, X.shape[0], X.shape[1],
                                          Xp.shape[1], int(X_orth))
    else:
...
cdef void _pyc_ortho_complement_float(
    ...
    cMatrixDecompositions[float].ortho_complement(
            c_X, c_Xp, num_rows, num_columns_X, num_columns_Xp, X_orth)
